"""MLP security — trend + trap-adjusted distribution yield + dollar volume + inverted 20d vol (no RSI)."""
...
_TECNICOS_PATH = CONFIG_DIR / "indicadores_tecnicos_energy_mlp.json"
...
def _load_security_ingredients() -> list[dict[str, Any]]:
    if not _TECNICOS_PATH.is_file():
        return [
            ...
            {"id": _YIELD_ID, "peso": 0.3, "inverte_percentil": False},
...
def _load_security_weights() -> dict[str, float]:
    ...
    if not _CONFIG_PATH.is_file():
        return {"wa": 0.3, "wb": 0.3, "wc": 0.2, "wd": 0.2}
...
def compute_energy_mlp_security_batch(
    ...
    for ticker in tickers:
        ...
        componentes = [
            ...
            {
                "id": _YIELD_ID,
                "nome": "Distribution yield (anti yield-trap)",
                "camada": "valuation",
                "valor": y_raw,
                ...
                "role": "renda vs pares após haircut; coverage DCF fica para 1B",
            },
            {
                "id": _VOLUME_ID,
        ...
        explanation = [
            f"SecurityScore = {security_score:.3f} (ranking MLP — não mistura com regime).",
