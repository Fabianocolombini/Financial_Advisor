"""IG security selection — duration-scaled technicals + duration fit vs term premium (Model 2)."""
...
_TECNICOS_PATH = CONFIG_DIR / "indicadores_tecnicos_ig.json"
_DURATION_PATH = CONFIG_DIR / "ig_duration_map.json"
...
def _effective_duration(ticker: str, duration_map: dict[str, float]) -> float:
    raw = duration_map.get(ticker.upper(), duration_map.get("__default__", 6.0))
...
def _term_premium_pct(as_of: dt.date, window: int = 1260) -> float:
    tp_fred = "THREEFYTP10"
    if _CONFIG_PATH.is_file():
        cfg = json.loads(_CONFIG_PATH.read_text(encoding="utf-8"))
        tp_fred = cfg.get("term_premium_fred", tp_fred)
    tp_series = _term_premium_series(tp_fred)
    tp_pct, _ = _percentile_0_1(tp_series, as_of, window)
    return tp_pct


def compute_ig_security_batch(
    tickers: list[str],
    ...
    tp_pct = _term_premium_pct(as_of)
    ...
    for ticker in tickers:
        ...
        dur_fit = 1.0 - abs(dur_pct - tp_pct)
        if invert_dur:
            dur_fit = 1.0 - dur_fit
        ...
        componentes = [
            {
                ...
                "role": "tendência por unidade de duration — não premia IGLB só por convexidade",
            },
            {
                "id": _RSI_ID,
            ...
            {
                "id": _DURATION_ID,
                "nome": "Duration fit vs term premium",
                "camada": "valuation",
                "valor": raw_dur.get(t),
                "percentile_cs": dur_pct,
                "term_premium_pct": tp_pct,
                "dur_fit": dur_fit,
                "peso": wd,
                "inverte_percentil": invert_dur,
                "contribuicao": c_dur,
                "role": "bucket macro — mesma faixa de duration, mesmo fit; não diferencia emissores",
            },
        ]
        ...
        explanation = [
            f"SecurityScore = {security_score:.3f} (ranking IG — não mistura com regime).",
            (
            ...
            (
                f"Duration fit (bucket): P(dur)={dur_pct:.0%}, TP_pct={tp_pct:.0%} "
