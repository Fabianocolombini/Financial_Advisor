"""US equity security selection — trend + RSI + dollar volume + inverted 20d vol (Model 2)."""
...
_TECNICOS_PATH = CONFIG_DIR / "indicadores_tecnicos_us_equity.json"
...
def _dollar_volume(ticker: str, as_of: dt.date) -> float:
    """close × shares traded as-of — liquidity in dollars, not share count."""
    ...
    if not row:
        return 0.0
    return float(row["close"] or 0) * float(row["volume"] or 0)
...
def compute_us_equity_security_batch(
    ...
    for ticker in tickers:
        ...
        componentes = [
            {
                ...
                "role": "tendência cross-sectional — percentil alto = mais acima das médias",
            },
            {
                "id": _RSI_ID,
                "nome": "RSI 14d",
            ...
            },
            {
                ...
                "vol_window": 20,
                "role": "desconto de swing — menor vol vs pares no mesmo dia; spike de mercado não pune um nome isolado",
            },
        ]
        ...
        explanation = [
            f"SecurityScore = {security_score:.3f} (ranking US equity — não mistura com regime).",
