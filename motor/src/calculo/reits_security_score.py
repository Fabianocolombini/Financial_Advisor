"""REITs security — trend + yield + volume − vol (no RSI) (Model 2)."""
...
def compute_reits_security_batch(
    ...
    wa, wb, wc, wd = float(sw.get("wa", 0.30)), float(sw.get("wb", 0.25)), float(sw.get("wc", 0.25)), float(sw.get("wd", 0.20))
    ...
    for ticker in tickers:
        ...
        comps = [
            ...
            {"id": "dividend_yield", "nome": "Yield", "percentile_cs": p_yield.get(t, 0.5), "peso": wb, "contribuicao": wb * p_yield.get(t, 0.5), "role": "yield"},
        ...
        results[t] = build_security_result(ticker=t, as_of=as_of, security_score=score, componentes=comps, model="reits_security_v1", universe_size=len(cs), explanation=[f"SecurityScore = {score:.3f}."])
    return results
